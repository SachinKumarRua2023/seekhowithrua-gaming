/**
 * Mobile Dev Mission - Game Engine
 * Mobile app themed game
 */

class MobileDevMissionGame {
  constructor() {
  ...
  getSectionColor() {
    switch(this.currentSection) {
      case 'basic': return '#61dafb';
      case 'intermediate': return '#ff9500';
      case 'advanced': return '#34c759';
  ...
  render() {
    ...
    this.nodes.forEach(node => {
      ...
      this.ctx.fillStyle = node.answered ? '#34c759' : node.color;
      this.ctx.beginPath();
  ...
  drawTouchParticles() {
    this.particles.forEach(p => {
      ...
      this.ctx.fillStyle = `rgba(97, 218, 251, ${p.alpha * pulse})`;
      this.ctx.beginPath();
  ...
  drawSectionIndicator() {
    const sectionNames = {
      'basic': 'UI COMPONENTS',
      'intermediate': 'STATE MGMT',
      'advanced': 'NATIVE MODS'
    };
  ...
  showQuiz(questionIndex) {
    ...
    if (questionIndex > 0) {
      ...
      if (this.nodes[questionIndex - 1].answered) {
        ...
        hintBox.style.borderColor = '#34c759';
      }
    } else {
      hintText.textContent = question.hint;
      hintBox.style.borderColor = '#61dafb';
    }
  ...
  checkSectionComplete() {
    ...
    if (answeredCount === questions.length) {
      ...
      text.textContent = `App deployed! ${this.getSectionName()} complete.`;
  ...
  nextSection() {
    ...
    if (currentIndex < 2) {
      ...
      if (this.currentLevel < 10) {
        ...
        alert('Mission Complete! You are a Mobile Dev Expert!');
      }
    }
  ...
  showExtraHint() {
    ...
    document.getElementById('hintBox').style.borderColor = '#61dafb';
...
function restartLevel() {
  if (confirm('Restart app build? Progress will be reset.')) {
